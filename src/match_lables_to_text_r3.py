...
import re
import sys
label_file = '../../data/democracy_reports_corpus_izabella_4.csv'
...
df_good.to_csv("result_r3.csv", index=False)
df_bad.to_csv("result_bad_r3.csv", index=False)

df = pd.read_csv('result_r3.csv')
df_filtered = df[df['dimension1'].notnull()]
labels = df_filtered['dimension1'].unique()
print(labels)
